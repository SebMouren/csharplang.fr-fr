...
- L’expression `this.Identifier` crée une liaison avec un membre d’instance du type englobant, qui doit être un champ ou une propriété ; les
- Le membre auquel il est lié (et l’accesseur `get`, s’il s’agit d’une propriété) est au moins aussi accessible que la méthode ; les
...
>     p = p.With(x: 1);
> ```
> Pour créer un `Point` de la même manière qu’un `Point` existant, mais avec la valeur de `X` modifiée.
> 
...
Consultez la [spécification de critères spéciaux](csharp-8.0/patterns.md#positional-pattern) pour une spécification de `Deconstruct` et sa relation à la mise en correspondance des modèles.

> **Remarques relatives**à la conception : en vertu de la `Deconstruct` générée par le compilateur, comme indiqué dans le présent document, et de la spécification de la mise en correspondance des modèles, une déclaration d’enregistrement
> ```cs
...
La syntaxe d’une déclaration `class` ou `struct` est étendue pour prendre en charge les paramètres de valeur ; les paramètres deviennent les propriétés du type :
...
Un type d’enregistrement a un constructeur de `public` dont la signature correspond aux paramètres de valeur de la déclaration de type. C’est ce que l’on appelle le *constructeur principal* pour le type et entraîne la suppression du *constructeur par défaut* déclaré implicitement.
...
Pour chaque paramètre d’enregistrement d’une déclaration de type d’enregistrement, il existe un membre de propriété `public` correspondant dont le nom et le type sont extraits de la déclaration de paramètre de valeur. Son nom est l' *identificateur* de l' *record_property_name*, s’il est présent, ou l' *identificateur* du *record_parameter* dans le cas contraire. Si aucune propriété publique concrète (c’est-à-dire non abstraite) avec un accesseur `get` et avec ce nom et ce type est explicitement déclarée ou héritée, elle est produite par le compilateur comme suit :

* Pour un *struct d’enregistrement* ou une *classe d’enregistrement*`sealed` :
 * Un champ de `readonly` `private` est produit sous la forme d’un champ de stockage pour une propriété `readonly`. Sa valeur est initialisée pendant la construction avec la valeur du paramètre de constructeur principal correspondant.
 * L’accesseur `get` de la propriété est implémenté pour retourner la valeur du champ de stockage.
 * Chaque accesseur `get` de la propriété virtuelle héritée correspondante est substitué.
...
Pour un *struct d’enregistrement* ou une *classe d’enregistrement*`sealed`, les implémentations des méthodes `object.GetHashCode()` et `object.Equals(object)` sont produites par le compilateur, sauf indication contraire de l’utilisateur.
...
- [] **Problème d’ouverture**: nous devons également spécifier que nous implémentons chaque `IEquatable<T>.Equals`.
...
- [] **Problème d’ouverture**: il a été proposé d’implémenter `operator ==` et `operator !=` pour les types d’enregistrements.
- [] **Problème d’ouverture**: devons-nous générer automatiquement une implémentation de `object.ToString`?

##### `Deconstruct`

Un type d’enregistrement a une méthode de `public` générée par le compilateur `void Deconstruct`, sauf si une signature est fournie par l’utilisateur. Chaque paramètre est un paramètre de `out` du même nom et du même type que le paramètre correspondant du type d’enregistrement. L’implémentation fournie par le compilateur de cette méthode doit assigner chaque paramètre `out` avec la valeur de la propriété correspondante.
...
Dans une classe d’enregistrement `abstract`, la méthode `With` fournie par le compilateur est abstraite. Dans un struct d’enregistrement ou une classe d’enregistrement sealed, la méthode `With` fournie par le compilateur est `sealed`. Dans le cas contraire, la méthode `With` fournie par le compilateur est’Virtual et son implémentation retourne une nouvelle instance produite en appelant le constructeur principal avec les paramètres comme arguments pour créer une nouvelle instance à partir des paramètres et retourner cette nouvelle instance.

- [] **Problème d’ouverture**: nous devons également spécifier dans quelles conditions nous remplaçons ou implémentons des méthodes de `With` virtuelles héritées ou des méthodes de `With` à partir d’interfaces implémentées.
- [] **Problème d’ouverture**: nous devrions indiquer ce qui se passe quand nous héritons d’une méthode de `With` non virtuelle.

> **Design Notes**: étant donné que les types d’enregistrements sont par défaut immuables, la méthode `With` permet de créer une nouvelle instance qui est la même qu’une instance existante, mais avec les propriétés sélectionnées en fonction de nouvelles valeurs. Par exemple, donné
> ```cs
> public class Point(int X, int Y);
> ```
...
> **Design Notes**: étant donné qu’un type d’enregistrement peut hériter d’un autre, et que cette implémentation de `Equals` n’est pas symétrique dans ce cas, elle n’est pas correcte. Nous proposons de mettre en œuvre l’égalité de la manière suivante :
> ```cs
>     public bool Equals(Pair other) // for IEquatable<Pair>
>     {
...
> ```
> Les enregistrements dérivés `override EqualityContract`. L’alternative la moins attrayante est de restreindre l’héritage.
...
Nous avons envisagé d’ajouter des *constructeurs principaux* dans C# 6. Bien qu’ils occupent la même surface syntaxique que cette proposition, nous avons constaté qu’ils ne relevaient pas des avantages offerts par les enregistrements.
